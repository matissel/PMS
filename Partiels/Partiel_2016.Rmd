...
&= n \int_{R^+} \frac{1}{x} \frac{x^{n-1}e^{-\frac{x}{\lambda}}}{\Gamma(n)\lambda^n} dx \newline
&= \frac{n}{\Gamma(n)\lambda^n} \int_{R+}x^{n-2}e^{-\frac{x}{\lambda}} dx \newline
\end{align}$$

On voit dans la dernière équation que l'on a une forme qui ressemble à la fonction Gamma d'Euler. 
$$\Gamma : z \int_0^{+\infty} t^{z-1}e^{-t} dt$$
$$\begin{align}
u = \frac{x}{\lambda} ; dx = \lambda du 
\end{align}$$

Donc, par changement de variable : 
$$\begin{align}
 E \Bigg[\frac{n}{\sum X_i} \Bigg] &= \frac{n}{\Gamma(n)\lambda^n} \int_{R+} \lambda^{n-2}u^{n-2}e^{-u}\lambda du \newline
 &= \frac{\lambda n}{\Gamma(n)\lambda^n}\int_{R+} u^{n-2}e^{-u}du \newline
 &= \frac{n}{\Gamma(n)\lambda} \Gamma(n-1) \newline
 &= \frac{(n-2)!}{(n-1)!}\frac{n}{\lambda} \newline
 &= \frac{n}{n-1}\frac{1}{\lambda}
\end{align}$$
...
$$\begin{align}
 E\Big[ \hat{\theta_n}\Big] = \frac{n}{n-1}\frac{1}{n}  \newline
 E \Big[\frac{n-1}{n}\hat{\theta_n}\Big] = \frac{1}{\lambda}
\end{align}$$
...
Il faudrait fixer la garantie à 0.451 milliers d'heures pour n'avoir que 10% des transistors à changer. 
